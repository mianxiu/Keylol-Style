...
(function () {
  ...
  function fetchHotImg(aNode) {
    ...
    if (url !== null) {
      fetch(url)
        ...
        .then(bodyText => {
          let idJson =
            bodyText.match(idJsonRegx) !== null
              ? JSON.parse(bodyText.match(idJsonRegx)[0].replace(idJsonRegx, "$1"))
              : null;

          let otherImg =
            bodyText.match(otherImgRegx) !== null ? bodyText.match(otherImgRegx)[0].replace(otherImgRegx, `$1`) : null;
  ...
  function moveIndexSubject() {
    ...
    let steamNode = $(`#wp > div:nth-child(8)`);
    let manufacturerNode = $(`#wp > div:nth-child(9)`);
    let gameNode = $(`#wp > div:nth-child(10)`);
    let questionNode = $(`#wp > div:nth-child(11)`);
    let forumNode = $(`#wp > div:nth-child(12)`);
  ...
  function setNavIcons() {
    ...
    // 提醒数量
    let mailBadge =
      mail !== null && mail.children.length > 0
        ? `<span class="badge">${mail.textContent.replace(/.*(\d+)/, "$1")}</span>`
        : "";
    let noticBadge =
      notic !== null && notic.children.length > 0
        ? `<span class="badge">${notic.textContent.replace(/.*(\d+)/, "$1")}</span>`
        : "";
  ...
  function renderPostListNav() {
    ...
    if ($(`.subforum`) !== null) {
      ...
      $All(tagAtag).forEach(node => {
        ...
        let postTagName =
          node.innerText.match(postTagNameRegx) !== null
        ...
        if (node.children !== null && node.innerHTML !== tagIconMatch(node.innerHTML)) {
          node.innerHTML = `<span>${tagIconMatch(
            postTagName
          )}<span class="post-tag-num">${postNum}</span></span><span class="tag-tip">${node.innerText}</span>`;
        }
      });
    }
  }
  ...
  function renderPostInfo() {
    ...
    function renderPostInfoChild(element, symbolKey) {
      if (element !== null) {
        element.innerHTML = `<div class="suforum-symbol">${symbolHTML(symbolKey)}<span class="subforum-info-tip">${
          element.innerHTML
        }</span></div>`;
      }
    }
    ...
    if (thread !== null) {
      ...
      if (subForumRightTitle !== null) {
        subForumRightTitle.insertBefore(
          $(`.subforum_left_title_left_down>div`),
          $(`.subforum_right_title`).children[0]
        );
      ...
      $All(`.mn-nav-right-control-panel a`).forEach(node => {
        for (const key in controlPanelRegex) {
          ...
          if (controlPanelRegex[key].test(node.href) == true || controlPanelRegex[key].test(node.innerHTML) == true) {
            node.innerHTML = `<span>${symbolHTML(symbolControlPanel[key])}</span><span class="control-panel-tip">${
              node.innerHTML
            }</span>`;
          }
        }
  ...
  function avatarImgUrl(suid, size) {
    ...
    let avatarNum =
      suidCache.length > 3
        ? ((suidCache[0] = suidCache[0].padStart(3, "0")), suidCache)
        : (suidCache.unshift(`000`), suidCache);
  ...
  function postListTrRender(trNode) {
    ...
    function subTag(subTagHtml) {
      ...
      if (subTagHtml.match(tagRegx.subJectRegx) !== null) {
        return `<div class="post-list-icn">${subTagHtml
          .match(tagRegx.subJectRegx)[0]
          .replace(tagRegx.subJectRegx, "$1")}</div>`;
      }
      return "";
    }
  ...
  function renderThreadLists() {
    let postListNodes =
      $All(`tbody[id*="thread"]>tr`).length !== 0
        ? $All(`tbody[id*="thread"]>tr`)
        : $All(`.pg_tag #ct > div > div.bm_c > table > tbody > tr`);
  ...
  function renderEditorPermission() {
    ...
    if (postFullEditor !== null || postIconNode != null || signHtml !== null) {
       ...
       */
      toFullNode !== null
        ? (toFullNode.innerHTML = `<span>${symbolHTML(
            symbolEditor.gaojimoshi
          )}</span><span class="editor-tip">切换高级模式</span>`)
        : null;

      // 加粗
      minB !== null
        ? (minB.innerHTML = `<span>${symbolHTML(symbolEditor.blod)}</span><span class="editor-tip">${
            $(`.fpd > a:first-child`).title
          }</span>`)
        : null;

      // 附件
      minAttl !== null
        ? (minAttl.innerHTML = `<span>${symbolHTML(
            symbolEditor.attchment
          )}</span><span class="editor-tip">上传附件</span>`)
        : null;

      // 涂鸦
      paint !== null
        ? (paint.innerHTML = `<span>${symbolHTML(symbolEditor.tuya)}</span><span class="editor-tip">${
            paint.innerText
          }</span>`)
        : null;
  ...
  function userTipCard() {
    ...
    let symbolUserCardCallback = function () {
      let callback = function () {
        ...
        friend.innerHTML = `<span>${symbolHTML(symbolUserCard.addfriend)}</span><span class="user-card-tip">${
          friend.innerText
        }</span>`;
        poke.innerHTML = `<span>${symbolHTML(symbolUserCard.hi)}</span></span><span class="user-card-tip">${
          poke.innerText
        }</span>`;
        pm.innerHTML = `<span>${symbolHTML(symbolUserCard.iconmail)}</span></span><span class="user-card-tip">${
          pm.innerText
        }</span>`;
      };
  ...
  function renderPostFavatar(post) {
    ...
    const postStatusRegx = {
      ...
      comments:
        /<th><p>(<a\s{0,}href="home\.php\?mod=space&amp;uid=\d{0,}&amp;do=thread&amp;type=reply&amp;view=me&amp;from=space".+?>.+?\/a>)<\/p>(.+?)<\/th>/gm,
    ...
    function favatarStatus() {
      ...
      for (const key in postStatusRegx) {
        if (postStatusRegx[key].test(favatarHTML) === true) {
          ...
          favatarStatusTemplate += elementMatch.replace(
            postStatusRegx[key],
            `
              <div>
  ...
  function movePostElement(post) {
    ...
    function postTamplate(
      ...
      let postSteamBarHTML =
        postSteamBar !== null ? `<div class="steam_connect_user_bar">${postSteamBar.innerHTML}</div>` : "";
            ...
            <div class="post-content" id="${pid}">
                <div class="post-content-top">
                   ...
                   ${manageBtn !== null ? manageBtn.innerHTML : ""}
                        ${
                          manageBtn !== null
                            ? `<label title="" for="${manageBtn.getAttribute(
                                `for`
                              )}" class="post-select-label">${symbolHTML(symbolSelect.selectnomalsvg)}</label>`
                            : ""
                        }
      ...
      fetch(url)
        ...
        .then(html => {
          ...
          let contentHTML =
            pmNode !== null
              ? pmNode.innerHTML
              : `<div class="locked">${postmessageDOM.querySelector(`.locked`).innerHTML}</div>`;
  ...
  function renderSympolSteamBar(post) {
    ...
    if (steamUserBar !== null) {
      if (steamUserBar.firstChild !== null) {
        ...
        steamName.innerHTML = `<span>${symbolHTML(symbolSteamBar.Steam_icon_logo_post)}</span><span>${
    ...
    if (popCl !== null) {
      ...
      for (const key in symbolPostBottomRegx) {
        if (symbolPostBottomRegx[key].test(popClHTML) === true) {
          let nodeMatch = popClHTML.match(symbolPostBottomRegx[key])[0].replace(
            symbolPostBottomRegx[key],
            `
                $1
    ...
    if (postTopBar !== null) {
      ...
      for (const key in postTopBarRegx) {
        if (postTopBarRegx[key].test(nodeHTML) == true) {
          postTopBarTemplate += nodeHTML.match(postTopBarRegx[key])[0].replace(
            postTopBarRegx[key],
            `$1
          <span>
  ...
  let pageDecide = function () {
    ...
    // 版块
    let isSubject = new RegExp(`(${keylolDomin}\/forum.php\\?mod=forumdisplay.*)|${keylolDomin}\/f\\d{3}.*`).test(
      currentHref
    );
    // 帖子
    let isPostContent = new RegExp(
      `(${keylolDomin}\/forum.php\\?mod=(viewthread|post).*)|(${keylolDomin}\/t\\d{3}.*)`
    ).test(currentHref);
